...
bool Bullet::init(Vec2 position, float speed, int dir)
{
	...
	m_texture = Director::getInstance()->getTextureCache()->addImage("Chapter12/tank/bullet3.png");
	switch (dir)
	{
	case BULLET_UP:
...
void Bullet::Blast()
{
	...
	// 粒子特效
	CCParticleSystemQuad* ps = CCParticleSystemQuad::create("ExplodingRing.plist");
	ps->setPosition(this->getPositionX(), this->getPositionY());
	ps->setStartSize(1);
	ps->setEndSize(1);
	this->getParent()->addChild(ps, 10);
